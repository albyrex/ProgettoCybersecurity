...
bool Connection::exchangeKey(const string& pathMyCert, X509** otherCert) {
	/*
	   Step 1. Creazione sessione Diffie-Hellman e invio di Ya (cioè la chiave pubblica, la Ya).
	*/
	vector<byte> Ya;
	vector<byte> a;
	...
	/*
	   Step 2. La Ya (chiave pubblica) appena mandata deve essere firmata, perciò
	...
	*/
	...
	vector<byte> sign = Signer::staticSign(my_priv, Ya);
